...
export default function ClientResetPasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    if (password !== confirm) {
      setError(t("clientResetPassword.mismatch"));
      return;
    }
    if (password.length < 8) {
      setError(t("clientResetPassword.tooShort"));
      return;
    }
    setSubmitting(true);
    try {
      await clientResetPassword(params.token, password);
      setDone(true);
      setTimeout(() => router.replace("/client/login"), 2000);
    } catch {
      setError(t("clientResetPassword.error"));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 px-4 dark:bg-black">
      ...
      <Card className="w-full max-w-sm p-6">
        ...
        <h1 className="mb-5 text-lg font-semibold text-zinc-900 dark:text-zinc-50">
          {t("clientResetPassword.title")}
        </h1>

        {done ? (
          <p className="text-sm text-emerald-700 dark:text-emerald-400">{t("clientResetPassword.success")}</p>
        ) : (
          <>
            {error && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <label className={labelClass}>
                {t("clientResetPassword.newPassword")}
                <input
                  type="password"
                  value={password}
              ...
              <label className={labelClass}>
                {t("clientResetPassword.confirmPassword")}
                <input
                  type="password"
                  value={confirm}
              ...
              <Button type="submit" className="w-full" disabled={submitting}>
                {submitting ? t("clientResetPassword.submitting") : t("clientResetPassword.submit")}
              </Button>
            </form>
          </>
        )}
        ...
        >
          {t("clientResetPassword.goToLogin")}
        </Link>
      </Card>
    </div>
  );
